    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-6">
          ...
          <button className="button-hover" onClick={onClose}>
            <IoClose size={24} />
          </button>
        </div>
            ...
            {project.details.screenshots && (
              <div className="w-full flex overflow-x-scroll gap-2">
                {project.details.screenshots.map((screenshot, index) => (
                  <Image key={index} src={screenshot} alt={`Screenshot ${index + 1}`} width={500} height={300} className="object-contain" />
                ))}
              </div>
            )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.mainTechStack.map((tech, index) => (
                        <span key={index} className="px-3 py-1 bg-background2 rounded-full text-sm">
                          {tech}
                        </span>
                      ))}
                    </div>
